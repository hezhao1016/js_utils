...
MzTreeView.prototype.focus = function(sourceId, defer, delaytime)
{
  if (!defer)
  {
  	if(!delaytime) delaytime = 100;
    setTimeout(this.name +".focus('"+ sourceId +"', true)", delaytime);
    return;
  }
...
{
  if(typeof(customMzTreeNodeDbclickHandler) == "function") {
	if(customMzTreeNodeDbclickHandler(e)) { // 返回true就是直接退出事件，否则继续往下走
		return ;
	}
  }
  e = window.event || e; e = e.srcElement || e.target;
...
{
  if (!this.currentNode) this.currentNode = this.node["0"];
  if(!this.currentNode.hasChild && this.node[this.currentNode.parentId].isExpand) {
	  this.focusClientNode(this.node[this.currentNode.parentId].childNodes[this.node[this.currentNode.parentId].childNodes.length-1].id);
  }
  if (this.currentNode.hasChild)
  {
    if (this.currentNode.isExpand) {
      this.focusClientNode(this.currentNode.childNodes[this.currentNode.childNodes.length-1].id);
	...
	for(var i=0; i<a.length; i++){
		if(a[i].id==e.id){
			if(i>0){
				...
				this.focusClientNode(e.id); return;
			} else {
...
MzTreeView.prototype.nextNode = function()
{
	...
	while(typeof(e.parentId)!="undefined") {
		if(e.parentId == "0" && e.childNodes.length > 0) {
		...
		for(var i=0; i<a.length; i++) { 
			if(a[i].id==e.id) {
				if(i<a.length-1) {
					this.focusClientNode(a[i+1].id); return;
 ...
 * 获取树的节点信息（节点总数和节点层级）
 */
MzTreeView.prototype.getTreeNodeInfo = function(size) {
	if(!size) size = 3;
	var obj = {total: 0, maxlevel: 1}; 
	var tmpstr = null;
	var tmplevel = 0;
	for(var x in this.nodes) {
		tmpstr = x.substring(x.indexOf("_") + 1);
		tmplevel = tmpstr.length / size;
		if(tmplevel > obj.maxlevel) {
			obj.maxlevel = tmplevel;
		}
		obj.total++;
	}
	return obj;
 ...
 * 展开当前的节点（如果有孩子节点的话）
 */
MzTreeView.prototype.expandCurNodeChild = function() {
	if(this.currentNode && this.currentNode.hasChild) {
 ...
 * 首次展开树，默认展开第3级的第一个节点
 */
MzTreeView.prototype.firstExpand = function() {
	// 默认首次展开到第3级菜单的第一个节点，没有则以此往上级展开
	var firstExpandLevel = 3;
	var nodeInfo = this.getTreeNodeInfo();
	if(nodeInfo.maxlevel >= firstExpandLevel) {
		this.nextNode();
		if(this.expandCurNodeChild()) {
			this.nextNode();
		} 
	}
	this.nodeClick(this.currentNode.id);
	this.expandCurNodeChild();	
};

/**
 * 展开到某个节点
 */
MzTreeView.prototype.openTo = function(id) {
	this.focus(id, true);
	this.expandCurNodeChild();
	this.nodeClick(this.currentNode.id);
};
 ...
 *			返回一个对象，对象形式：{no: ..., pno: ...}
 */
MzTreeView.prototype.getCurrentNodeNo = function() {
	var nostr = this.currentNode.sourceIndex;
	var obj = {no: '', pno: ''};
	var idx = nostr.indexOf("_");
	if(idx > -1) {
		obj.no = nostr.substring(idx+1);
		obj.pno = nostr.substring(0, idx);
	}
	return obj;
};
 ...
 * 首次展开树
 */
MzTreeView.prototype.firstMyExpand = function(target) {
	var nodeInfo = this.getTreeNodeInfo();
	this.nodeClick(this.currentNode.id);
	if(target) {
	    this.expandCurNodeChild();
 ...
 * 返回当前的节点的完整的文本全路径
 */
MzTreeView.prototype.getPathText = function(no) {
	var nodeNodArr = this.getPath(no);
	var pathStr = "";
	var txt = null;
	var sourceIndex = "0_";
	for(var i = 1; i < nodeNodArr.length; i++) {
		if(i > 1) {
			sourceIndex = nodeNodArr[i-1] + "_" + nodeNodArr[i];
		} else {
			sourceIndex += nodeNodArr[i];
		}
		txt = this.getAttribute(this.nodes[sourceIndex], "text");
		if(i == nodeNodArr.length - 1) {
			pathStr += txt;
		} else {
			pathStr += txt + "/";
		}
...
function createMzTree(treename, iconPath) {
	if(typeof(treename) == "undefined") {
		treename = "tree";
	}	
	if(typeof(iconPath) == "undefined") {
		iconPath = "../js/tree/images/";
	}
	var _tree = new MzTreeView(treename);
	_tree.icons["root"] = "unit.gif";
	_tree.icons["folder"] = "unit.gif";
	_tree.icons["file"] = "unit.gif";
	_tree.iconsExpand["folder"] = "unit.gif";
	_tree.setIconPath(iconPath);
	return _tree;
}